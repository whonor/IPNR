...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        if attn_mask is not None:
            attn_mask = self.att_fc2(attn_mask)
...
class FastSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        attention_mask = self.query_att(attention_mask).transpose(1, 2)
        # add attention mask
...
class FastformerEncoder(nn.Module):
    def __init__(self, config, pooler_count=1):
        ...
        self.linear = nn.Linear(config.hidden_size, config.num_attention_heads)
    ...
    def forward(self,
        ...
        attention_mask = input_embs.bool().float()
