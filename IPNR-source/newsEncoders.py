...
class IPNR(NewsEncoder):
    def __init__(self, config: Config):
        super(IPNR, self).__init__(config)
...
class MHSA(NewsEncoder):
    def __init__(self, config: Config):
        ...
        self.C_multiheadAttention = MultiHeadAttention(config.head_num, config.word_embedding_dim,
                                                     config.max_abstract_length, config.max_abstract_length, config.head_dim,
                                                     config.head_dim)
        self.C_attention = Attention(config.head_num * config.head_dim, config.attention_dim)
        self.news_embedding_dim = config.head_num * config.head_dim
        self.category_affine = nn.Linear(in_features=config.category_embedding_dim, out_features=self.feature_dim,
                                         bias=True)
        self.subCategory_affine = nn.Linear(in_features=config.subCategory_embedding_dim,
                                            out_features=self.feature_dim, bias=True)

        self.affine1 = nn.Linear(in_features=self.feature_dim, out_features=config.attention_dim, bias=True)
        self.affine2 = nn.Linear(in_features=config.attention_dim, out_features=1, bias=False)
    ...
    def initialize(self):
        ...
        self.attention.initialize()
        self.C_multiheadAttention.initialize()
        self.C_attention.initialize()
    ...
    def forward(self, title_text, title_mask, title_entity, content_text, content_mask, content_entity, category, subCategory, user_embedding):
        ...
        news_num = title_text.size(1)
        batch_news_num = batch_size * news_num
        mask = title_mask.view([batch_news_num, self.max_title_length])                                                          # [batch_size * news_num, max_sentence_length]
        content_mask = content_mask.view([batch_news_num, self.max_content_length])
        # 1. word embedding
        title_vector = self.dropout(self.word_embedding(title_text)).view([batch_news_num, self.max_title_length, self.word_embedding_dim]) # [batch_size * news_num, max_sentence_length, word_embedding_dim]
        # 2. multi-head self-attention
        title_feature_vector = self.dropout(self.multiheadAttention(title_vector, title_vector, title_vector, mask))                                                                    # [batch_size * news_num, max_sentence_length, news_embedding_dim]
        # 3. attention layer
        title_representation = self.attention(title_feature_vector, mask=mask).view([batch_size, news_num, self.feature_dim])                           # [batch_size, news_num, news_embedding_dim]
        # 1. word embedding
        content_vector = self.dropout(self.word_embedding(content_text)).view([batch_news_num, self.max_content_length,
                                                                           self.word_embedding_dim])  # [batch_size * news_num, max_sentence_length, word_embedding_dim]
        # 2. multi-head self-attention
        content_feature_vector = self.dropout(self.C_multiheadAttention(content_vector, content_vector, content_vector,
                                                                    content_mask))  # [batch_size * news_num, max_sentence_length, news_embedding_dim]
        # 3. attention layer
        content_representation = self.C_attention(content_feature_vector, mask=content_mask).view(
            [batch_size, news_num, self.feature_dim])

        # 4. category and subCategory encoding
        category_representation = F.relu(self.category_affine(self.category_embedding(category)),
                                         inplace=True)  # [batch_size, news_num, news_embedding_dim]
        subCategory_representation = F.relu(self.subCategory_affine(self.subCategory_embedding(subCategory)),
                                            inplace=True)
        # 5. multi-view attention
        feature = torch.stack(
            [title_representation, content_representation, category_representation, subCategory_representation],
            dim=2)  # [batch_size, news_num, 4, news_embedding_dim]
        alpha = F.softmax(self.affine2(torch.tanh(self.affine1(feature))), dim=2)  # [batch_size, news_num, 4, 1]
