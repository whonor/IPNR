...
class Model(nn.Module):
    def __init__(self, config: Config):
        ...
        elif config.news_encoder == 'IPNR':
            self.news_encoder = newsEncoders.IPNR(config)
        else:
            raise Exception(config.news_encoder + 'is not implemented')

        # For main experiments of user encoding
        if config.user_encoder == 'IPNR':
            self.user_encoder = userEncoders.IPNR(self.news_encoder, config)
    ...
    def forward(self, user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_history_mask, user_history_graph, user_concept_text, user_concept_mask,\
                      news_category, news_subCategory, news_title_text, news_title_mask, news_title_entity, news_content_text, news_content_mask, news_content_entity, news_concept_text, news_concept_mask):
        ...
        user_representation = self.user_encoder(user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_category, user_subCategory, \
                                                user_history_mask, user_history_graph, user_concept_text, user_concept_mask, user_embedding, news_representation)                           # [batch_size, 1 + negative_sample_num, news_embedding_dim]
