...
from layers import MultiHeadAttention, Attention, ScaledDotProduct_CandidateAttention, CandidateAttention, GCN, Conv1D
...
class UserEncoder(nn.Module):
    def __init__(self, news_encoder: NewsEncoder, config: Config):
        ...
        self.pretrained_concept_embedding = torch.from_numpy(
            np.load(
                '../MIND-small/all_concept_word_embedding.npy')).float().to(device=self.device)    # build by 03_generate_concept_embedding.py
...
class IPNR(UserEncoder):
    ...
    def forward(self, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_category, user_subCategory, \
        ...
        return inter_cluster_feature  # [batch_size, news_num, news_embedding_dim]
